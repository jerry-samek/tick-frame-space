...
def compute_gamma_grav(
    ...
    Args:
        L: Load field
        E: Energy field
        capacity_min: Minimum effective capacity (ignored if allow_divergence=True)
        work_threshold: Minimum energy required for work
        allow_divergence: If True, allow gamma → ∞ (for black hole testing)
    ...
    if not allow_divergence:
        # Standard mode: cap minimum capacity (gamma_max = 10)
        capacity_eff = np.maximum(capacity_eff, capacity_min)
    else:
        # Black hole mode: allow capacity → 0 (gamma → ∞)
        # Use tiny epsilon to prevent true division by zero
        capacity_eff = np.maximum(capacity_eff, 1e-6)
