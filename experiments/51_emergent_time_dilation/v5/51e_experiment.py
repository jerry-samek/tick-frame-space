#!/usr/bin/env python3
"""
Experiment 51e: Stabilized Saturation Field
-------------------------------------------
...
class Config:
    ...
    planet_tick_budget: int = 2
    space_tick_budget: int = 1

    base_capacity: float = 1.0
    capacity_sensitivity: float = 0.05
    capacity_min: float = 0.05

    diffusion_alpha: float = 0.01
    diffusion_beta: float = 0.005
...
class Simulation51e:
    ...
    def setup(self):
        print(f"Setting up Experiment 51e [{self.cfg.name}]...")
    ...
    def diffuse_load(self, L):
        ...
        # damped diffusion
        L_new = (1 - self.cfg.diffusion_beta) * L + self.cfg.diffusion_alpha * lap
        return np.clip(L_new, 0.0, None)
    ...
    def run(self):
        ...
        for tick in range(self.cfg.num_substrate_ticks):
            ...
            # 2) compute load and diffuse
            raw = self.compute_raw_load()
            self.load_field = self.diffuse_load(raw)

            # 3) compute effective capacity
            C_eff = self.effective_capacity(self.load_field)

            # 4) apply capacity to cells
            for (i, j), cell in self.cells.items():
                cap = C_eff[i, j]
...
if __name__ == "__main__":
    ...
    # Example: batch run
    configs = [
        Config(diffusion_alpha=0.005, name="alpha_0.005"),
        Config(diffusion_alpha=0.01, name="alpha_0.01"),
        Config(diffusion_alpha=0.02, name="alpha_0.02"),
    ]
