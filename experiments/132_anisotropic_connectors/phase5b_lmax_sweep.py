#!/usr/bin/env python3
"""
Experiment 132 Phase 5b — Loosen the L_MAX cap, re-test horizon scaling

Phase 5 found r_horizon insensitive to STAR_COUNT (identical to 4 decimals
across 8x range) and only weakly sensitive to L_STAR. Diagnosis: L_MAX=0.95
cap was pinning the horizon at the cluster boundary, not letting source
strength push the saturated region outward.

Test: re-run with L_MAX=0.99. If r_horizon now scales with source, the cap
was the artifact. If still flat, the substrate genuinely has a stiff
horizon scale set by graph discreteness.

Sweeps with L_MAX=0.99:
  L_STAR ∈ {0.5, 1.0, 1.5, 2.0}     (STAR_COUNT=50)
  STAR_COUNT ∈ {25, 50, 100, 200}   (L_STAR=1.0)

Add threshold L_t=0.95 since field can now reach higher values.
"""
...
def find_horizons(rho, r_of, thresholds):
  bin_edges = np.logspace(np.log10(0.3), np.log10(0.9 * SPHERE_R), 80)
  ...
  L_max_observed = float(L_shell.max())
  horizons = {}
  for L_t in thresholds:
    ...
    for i in range(1, len(centers)):
      if L_shell[i] < L_t and L_shell[i - 1] >= L_t:
        ...
        break
  return horizons, centers, L_shell, L_max_observed
...
def report_sweep(label, results, sweep_values):
  print("\n" + "=" * 70)
  print(label)
  print("=" * 70)
  print(f"  {'param':>7}  {'L_max_obs':>10}  " +
        "  ".join([f'r_h(L={L_t})' for L_t in THRESHOLDS]))
  for x, hor, _, _, lmax_obs in results:
    row = "  ".join([f'{hor.get(L_t, float("nan")):>10.3f}' for L_t in THRESHOLDS])
    print(f"  {x:>7.2f}  {lmax_obs:>10.4f}  {row}")


def report_fits(L_results, SC_results):
  print("\n" + "=" * 70)
  print("SCALING FITS (L_MAX=0.99)")
  print("=" * 70)
...
def plot_results(L_results, SC_results):
  ...
  ax = axes[0, 0]
  for l_star, _, centers, L_shell, _ in L_results:
  ...
  ax.set_title(f'L(r) vs L_STAR (L_MAX=0.99, STAR_COUNT=50)')
  ax.legend(fontsize=8); ax.grid(True, which='both', alpha=0.3)

  ax = axes[0, 1]
  for sc, _, centers, L_shell, _ in SC_results:
    ax.loglog(centers, np.maximum(L_shell, 1e-12), 'o-', ms=3,
              label=f'STAR_COUNT={sc}')
  for L_t in THRESHOLDS:
    ax.axhline(L_t, color='gray', linestyle=':', alpha=0.4)
  ax.set_xlabel('r'); ax.set_ylabel('L(r)')
  ax.set_title(f'L(r) vs STAR_COUNT (L_MAX=0.99, L_STAR=1.0)')
  ax.legend(fontsize=8); ax.grid(True, which='both', alpha=0.3)
  ...
  ax.set_title('r_horizon vs L_STAR')
  ...
  ax.set_title('r_horizon vs STAR_COUNT')
  ax.legend(fontsize=8); ax.grid(True, which='both', alpha=0.3)

  plt.suptitle(f'Exp 132 Phase 5b: horizon mapping with L_MAX={L_MAX_NEW}',
               fontsize=11)
  plt.tight_layout()
  path = os.path.join(OUT, 'phase5b_lmax_sweep.png')
  plt.savefig(path, dpi=130); plt.close()
  print(f"\nSaved: {path}")


def run():
  print(f"Phase 5b: horizon mapping with L_MAX={L_MAX_NEW}")
  print(f"  (Phase 5 used L_MAX=0.95)")
  print()
  ...
  for l_star in L_STAR_VALUES:
    print(f"=== L_STAR={l_star} STAR_COUNT=50 L_MAX={L_MAX_NEW} ===")
    t0 = time.time()
    rho, r_of, bm, _ = converge_with(pos, src, dst, edge_dir, edge_len,
                                       l_star, 50, L_MAX_NEW)
    horizons, centers, L_shell, lmax_obs = find_horizons(rho, r_of, THRESHOLDS)
    print(f"  ({time.time()-t0:.1f}s)  L_max_obs={lmax_obs:.4f}  horizons = " +
          ", ".join([f"L={L_t}: r={horizons.get(L_t, float('nan')):.3f}"
                     for L_t in THRESHOLDS]))
    L_results.append((l_star, horizons, centers, L_shell, lmax_obs))
    sys.stdout.flush()

  SC_results = []
  for sc in STAR_COUNT_VALUES:
    print(f"=== STAR_COUNT={sc} L_STAR=1.0 L_MAX={L_MAX_NEW} ===")
    t0 = time.time()
    rho, r_of, bm, _ = converge_with(pos, src, dst, edge_dir, edge_len,
                                       1.0, sc, L_MAX_NEW)
    horizons, centers, L_shell, lmax_obs = find_horizons(rho, r_of, THRESHOLDS)
    print(f"  ({time.time()-t0:.1f}s)  L_max_obs={lmax_obs:.4f}  horizons = " +
          ", ".join([f"L={L_t}: r={horizons.get(L_t, float('nan')):.3f}"
                     for L_t in THRESHOLDS]))
    SC_results.append((sc, horizons, centers, L_shell, lmax_obs))
    sys.stdout.flush()

  report_sweep("L_STAR SWEEP RESULTS (STAR_COUNT=50 fixed)", L_results, L_STAR_VALUES)
  report_sweep("STAR_COUNT SWEEP RESULTS (L_STAR=1.0 fixed)", SC_results, STAR_COUNT_VALUES)
