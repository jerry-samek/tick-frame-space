...
*Correction to the banked P1c (2026-07-11, user-driven, recorded as RAW 134 Addendum F §18.2): the PASS target is re-specified from absolute population stationarity to **comoving stationarity** — polynomial-in-time growth with a stationary shell exponent and decaying boundary fraction. Absolute stationarity is a freeze (both frozen states produced in Exp 138 read geometry-arrested/ambiguous); the geometric phase grows, hidden by scale. Growth-shape re-analysis of the P1b data under this lens: 100/100 analyzable trajectories exponential-in-time (`results_p1_growthshape_console.txt`) — the registered negative survives its own correction.*
