...
- **Banked (revised):** a **fixture-independent-window** ball-growth (stop at a fixed cone-coverage fraction, not a fixed R_max), with a **deeper tree** and a **same-N (~2300) random-4-regular** added to the null zoo, before any absolute-dimension claim is trusted.

## Provenance
Prereg frozen `1c21e61`; channel `159611c`; gate hardened `7002e78`; wiring+sweeps `962c0ee`; (q,c)-map `002a766`; ball-growth instrument `e4bfd14`. **Three** fresh-context skeptic passes: (1) gate/probe recon → A.8; (2) this channel result → refuted the operator's (b), established qualified (a); (3) the ball-growth dimension instrument → refuted it as a trustworthy reader, withdrew "≥3D", salvaged the frac_exp>0 heterogeneity signal. The recurring pattern (three operator overclaims caught) is the discipline working; the dimension of the manifold regime remains genuinely open.
