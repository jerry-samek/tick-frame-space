...
v17 showed every G=0 regime fails: fossil orbits die (gradient disperses),
Eddington peak is a mirage (self-blinding at long timescales), radiation
kills mass too fast. The formation phase was artificial — pre-built gamma wells.

v18 removes the formation phase entirely. Entities deposit gamma from tick 1,
building their own gravitational field in real time. The gradient emerges from
the balance of continuous source (deposit) vs dilution (expansion). No fossil.
No pre-built wells. Just expansion and radiation.

Two mechanisms only: expansion (edges grow, suppressed by gamma) and radiation
(entities deposit gamma, losing mass). G=0 always — no self-gravitating spread.

If topology-only spread fails to maintain gradients (gamma diffuses equally
on short and long edges), add edge-weighted spread: weight = initial_length /
current_length. Not a new mechanism — just making diffusion honest about the
space it's diffusing through.

Key parameters: mass=100000, deposit_rate=0.00001 -> deposits 1.0 gamma/tick,
half-life ~70K ticks. Mass survives long enough for multiple orbits while
maintaining a gradient that reaches orbital distance.

Usage:
    python macro_bodies.py --verify
    python macro_bodies.py --phase0 --n-nodes 30000 --ticks 50000
    python macro_bodies.py --phase1 --n-nodes 30000 --ticks 50000
    python macro_bodies.py --phase2 --n-nodes 30000 --ticks 50000
    python macro_bodies.py --sweep-deposit --n-nodes 30000 --ticks 50000
...
def _avg_gamma_at_r(gamma, node_dists, r, width=1.0):
    """Average gamma in a radial shell at distance r."""
    mask = (node_dists > r - width) & (node_dists < r + width)
    if np.any(mask):
        return float(np.mean(gamma[mask]))
    return 0.0
...
def experiment_phase0(n_nodes=10000, k=12, H=0.01, alpha_expand=1.0,
    ...
    gradient at orbital distances (r=10-20)? Topology-only spread.
    """
    print("=" * 70)
    print("PHASE 0: Gradient Formation Test (topology-only spread)")
    print("  Single body deposits gamma from tick 1. No formation phase.")
    print("  Does the gradient reach orbital distances?")
    print("=" * 70)
    ...
    body = Entity('A', center, mass=mass, deposit_rate=deposit_strength,
                  inertia=1.0, stationary=True, radiate_mass=radiate_mass)

    e0_mean = np.mean(graph.edge_lengths)
    print(f"\n  Graph: N={n_nodes}, k={k}, mean_edge={e0_mean:.2f}")
    print(f"  Mass={mass}, deposit_rate={deposit_strength}")
    print(f"  Deposit/tick={mass * deposit_strength:.2f}")
    print(f"  Half-life={0.693 / deposit_strength:.0f} ticks")
    print(f"  H={H}, radiate_mass={radiate_mass}")

    # Snapshot gamma profiles at several times
    snapshot_ticks = [1000, 5000, 10000, 25000, ticks]
    ...
    for tick in range(ticks):
        ...
        if (tick + 1) in snapshot_ticks:
            ...
            # Gamma profile from body's node
            center_pos = graph.positions[body.node]
            node_dists = np.linalg.norm(graph.positions - center_pos, axis=1)

            profile = {}
            for pr in probe_radii:
                mask = (node_dists > pr - 1.0) & (node_dists < pr + 1.0)
                if np.any(mask):
                    profile[pr] = float(np.mean(graph.gamma[mask]))
                else:
                    profile[pr] = 0.0
            profiles[tick + 1] = profile

            gamma_str = " ".join(
                f"r={pr}:{profile[pr]:.3f}" for pr in probe_radii)
            print(f"    Tick {tick+1:6d}: mass={body.mass:.1f} a={a_scale:.1f} "
                  f"gamma=[{gamma_str}] peak={graph.gamma[body.node]:.2f} "
                  f"({elapsed:.1f}s)")

    # Plot: gamma profiles at different times
    suffix = f"_{tag}" if tag else ""
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 6))

    for t, profile in sorted(profiles.items()):
        rs = sorted(profile.keys())
        gs = [profile[r] for r in rs]
        ax1.plot(rs, gs, 'o-', markersize=6, label=f't={t}')
    ax1.set_xlabel('Distance from body (Euclidean)')
    ax1.set_ylabel('Average gamma')
    ax1.set_title('Gamma Profile vs Distance')
    ax1.legend()
    ...
    for t, profile in sorted(profiles.items()):
        rs = sorted(profile.keys())
        gs = [max(profile[r], 1e-10) for r in rs]
        ax2.loglog(rs, gs, 'o-', markersize=6, label=f't={t}')
    # Reference 1/r line
    r_ref = np.linspace(1, 20, 50)
    g_ref = profiles[snapshot_ticks[-1]].get(1.0, 1.0) / r_ref
    ax2.loglog(r_ref, g_ref, 'k--', alpha=0.3, label='1/r reference')
    ax2.set_xlabel('Distance (log)')
    ax2.set_ylabel('Gamma (log)')
    ax2.set_title('Gamma Profile (log-log, compare to 1/r)')
    ax2.legend()
    ax2.grid(True, alpha=0.3)

    fig.suptitle(f'Phase 0: Gradient Formation (M={mass}, dep={deposit_strength}, '
                 f'H={H})', fontweight='bold')
    fig.tight_layout()
    fig.savefig(RESULTS_DIR / f'phase0_gradient{suffix}.png', dpi=150)
    plt.close(fig)

    print(f"\n  Final mass: {body.mass:.1f}")
    print(f"\n  Saved: {RESULTS_DIR / f'phase0_gradient{suffix}.png'}")
    return profiles
...
def experiment_phase1(n_nodes=10000, k=6, H=0.1, alpha_expand=1.0,
                      star_mass=100000.0, planet_mass=1.0,
                      deposit_strength=0.00001,
                      separation=10.0,
    ...
    # Warm-up: entities deposit and gamma spreads, but no movement
    if warm_up > 0:
        print(f"  Warm-up: {warm_up} ticks (deposit + spread, no movement)")
        wu0 = time.time()
        for wt in range(warm_up):
            ...
            graph.spread()
            graph.expand_edges()
        print(f"  Warm-up done in {time.time()-wu0:.1f}s")
        print(f"  Gamma: A_node={graph.gamma[bodies[0].node]:.1f}, "
              f"B_node={graph.gamma[bodies[1].node]:.1f}")
        print(f"  Mass: A={bodies[0].mass:.1f}, B={bodies[1].mass:.1f}")
    ...
    if any(b.pos_history for b in bodies):
            ...
            if body.pos_history:
                ...
                for i in range(len(xs)-1):
                    ax.plot(xs[i:i+2], ys[i:i+2], zs[i:i+2],
                            color=plt.cm.viridis(ts[i]), linewidth=0.8, alpha=0.7)
                ax.scatter(xs[0], ys[0], zs[0], c='green', s=80, marker='o', label=f'{body.bid} start')
                ax.scatter(xs[-1], ys[-1], zs[-1], c='red', s=80, marker='s', label=f'{body.bid} end')
        ax.set_title('Physical 3D')
        ...
        for body in bodies:
            if body.pos_history:
                xs_c, ys_c, zs_c = [], [], []
                for t, x, y, z in body.pos_history:
                    a_t = 1.0
                    for r in records:
                        if r['tick'] >= t:
                            a_t = r['a_scale']
                    ...
                    ys_c.append(y / a_t)
                    zs_c.append(z / a_t)
                ts = np.linspace(0, 1, len(xs_c))
                for i in range(len(xs_c)-1):
                    ax.plot(xs_c[i:i+2], ys_c[i:i+2], zs_c[i:i+2],
                            color=plt.cm.viridis(ts[i]), linewidth=0.8, alpha=0.7)
                ax.scatter(xs_c[0], ys_c[0], zs_c[0], c='green', s=80, marker='o')
                ax.scatter(xs_c[-1], ys_c[-1], zs_c[-1], c='red', s=80, marker='s')
        ax.set_title('Comoving 3D')
        ax.set_xlabel('X'); ax.set_ylabel('Y'); ax.set_zlabel('Z')

        fig.suptitle('Phase 1: 3D Trajectories (color = time)', fontweight='bold')
        fig.tight_layout()
...
def experiment_phase2(n_nodes=10000, k=12, H=0.1, alpha_expand=1.0,
                      mass=100000.0, deposit_strength=0.00001,
    ...
    for tick in range(ticks):
        ...
        if (tick + 1) % diag_interval == 0:
            ...
            a_scale = np.mean(graph.edge_lengths) / e0_mean
            d_comov = d / a_scale
            center_pos_diag = graph.positions[bodies[0].node]
            node_dists_diag = np.linalg.norm(graph.positions - center_pos_diag, axis=1)
            gamma_at_r1 = _avg_gamma_at_r(graph.gamma, node_dists_diag, 1.0)
            gamma_at_r5 = _avg_gamma_at_r(graph.gamma, node_dists_diag, 5.0)
            gamma_at_r10 = _avg_gamma_at_r(graph.gamma, node_dists_diag, 10.0)
            records.append({
                'tick': tick + 1, 'd_AB': d,
                'd_comov': d_comov, 'a_scale': a_scale,
                ...
                'gamma_r1': gamma_at_r1, 'gamma_r5': gamma_at_r5,
                'gamma_r10': gamma_at_r10,
            })
            ...
            if (tick + 1) % print_interval == 0:
                ...
                for body in bodies:
                    bpos = graph.positions[body.node]
                    bdists = np.linalg.norm(graph.positions - bpos, axis=1)
                    gamma_str = ""
                    for pr in [1.0, 5.0, 10.0, 20.0]:
                        mask = (bdists > pr - 1.0) & (bdists < pr + 1.0)
                        if np.any(mask):
                            gamma_str += f" r={pr:.0f}:{float(np.mean(graph.gamma[mask])):.3f}"
                    print(f"      {body.bid} gamma:{gamma_str}")
    ...
    # Plot: physical AND comoving distances + mass + velocity
    if records:
        fig, (ax1, ax2, ax3, ax4, ax5) = plt.subplots(5, 1, figsize=(14, 18))
        ticks_arr = [r['tick'] for r in records]
        ax1.plot(ticks_arr, [r['d_AB'] for r in records], 'b-', linewidth=1,
                 label='Physical distance')
        ax1.axhline(y=init_dist, color='gray', linestyle='--', alpha=0.3)
        ...
        ax5.plot(ticks_arr, [r['gamma_r1'] for r in records], 'b-', label='r=1')
        ax5.plot(ticks_arr, [r['gamma_r5'] for r in records], 'g-', label='r=5')
        ax5.plot(ticks_arr, [r['gamma_r10'] for r in records], 'r-', label='r=10')
        ax5.set_xlabel('Tick')
        ax5.set_ylabel('Gamma')
        ax5.set_title('Gamma Profile Over Time (from body A)')
        ax5.legend()
    ...
    if any(b.pos_history for b in bodies):
        ...
        fig.suptitle('Phase 2: 3D Trajectories (color = time)', fontweight='bold')
        fig.tight_layout()
...
def experiment_sweep_deposit(n_nodes=10000, k=12, H=0.01, alpha_expand=1.0,
    ...
    deposit_rates = [0.000001, 0.000005, 0.00001, 0.00005, 0.0001]
    results = []

    for dr in deposit_rates:
        print(f"\n{'='*40}")
        print(f"=== deposit_rate={dr} ===")
        print(f"{'='*40}")
        ...
        node_b = graph.nearest_node(np.array([half_sep, 0.0, 0.0]))
        e0_mean = np.mean(graph.edge_lengths)

        bodies = [
            Entity('A', node_a, mass=mass, deposit_rate=dr,
                   inertia=inertia, radiate_mass=True),
            Entity('B', node_b, mass=mass, deposit_rate=dr,
                   inertia=inertia, radiate_mass=True),
        ]
        bodies[0].set_velocity(np.array([0.0, tangential_momentum, 0.0]))
        bodies[1].set_velocity(np.array([0.0, -tangential_momentum, 0.0]))
        ...
        for tick in range(ticks):
            ...
            if (tick + 1) % max(ticks // 500, 1) == 0:
                d = graph.euclidean_distance(bodies[0].node, bodies[1].node)
                a_scale = np.mean(graph.edge_lengths) / e0_mean
                dists_c.append(d / a_scale)
            ...
            if (tick + 1) % print_interval == 0:
                ...
                mb = bodies[1].get_momentum_3d(graph)
                elapsed = time.time() - t0
                print(f"    Tick {tick+1:7d}: d={d:.1f} d_c={d/a_scale:.2f} "
                      f"mA={bodies[0].mass:.1f} "
                      f"|vA|={np.linalg.norm(ma):.3f} |vB|={np.linalg.norm(mb):.3f} "
                      f"hops={bodies[0].hops}+{bodies[1].hops} ({elapsed:.1f}s)")

        # Compute final metrics
        final_d = graph.euclidean_distance(bodies[0].node, bodies[1].node)
        final_a = np.mean(graph.edge_lengths) / e0_mean
        final_vA = np.linalg.norm(bodies[0].velocity)
        final_vB = np.linalg.norm(bodies[1].velocity)

        # Gamma profile
        center_pos = graph.positions[bodies[0].node]
        node_dists = np.linalg.norm(graph.positions - center_pos, axis=1)
        gamma_r5 = _avg_gamma_at_r(graph.gamma, node_dists, 5.0)
        gamma_r10 = _avg_gamma_at_r(graph.gamma, node_dists, 10.0)

        reversals = 0
        if len(dists_c) > 2:
            reversals = sum(1 for i in range(2, len(dists_c))
                            if (dists_c[i] - dists_c[i-1]) *
                               (dists_c[i-1] - dists_c[i-2]) < 0)

        results.append({
            'deposit_rate': dr,
            'final_vA': final_vA, 'final_vB': final_vB,
            'final_mass': bodies[0].mass,
            'reversals': reversals,
            'hops_A': bodies[0].hops, 'hops_B': bodies[1].hops,
            'gamma_r5': gamma_r5, 'gamma_r10': gamma_r10,
        })
    ...
    print("SWEEP RESULTS")
    print(f"{'='*70}")
    print(f"  {'deposit_rate':>12} {'|vA|':>8} {'|vB|':>8} {'mass':>10} "
          f"{'revers':>7} {'hops':>10} {'g_r5':>8} {'g_r10':>8}")
    for r in results:
        print(f"  {r['deposit_rate']:12.6f} {r['final_vA']:8.4f} {r['final_vB']:8.4f} "
              f"{r['final_mass']:10.1f} {r['reversals']:7d} "
              f"{r['hops_A']+r['hops_B']:10d} "
              f"{r['gamma_r5']:8.4f} {r['gamma_r10']:8.4f}")

    # Plot: velocity vs deposit_rate
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 6))

    drs = [r['deposit_rate'] for r in results]
    ax1.semilogx(drs, [r['final_vA'] for r in results], 'bo-', label='|vA|')
    ax1.semilogx(drs, [r['final_vB'] for r in results], 'rs-', label='|vB|')
    ax1.set_xlabel('deposit_rate (log)')
    ax1.set_ylabel('Final velocity')
    ax1.set_title('Velocity vs Deposit Rate')
    ax1.legend()
    ax1.grid(True, alpha=0.3)

    ax2.semilogx(drs, [r['reversals'] for r in results], 'go-')
    ax2.set_xlabel('deposit_rate (log)')
    ax2.set_ylabel('Comoving reversals')
    ax2.set_title('Orbital Stability vs Deposit Rate')
    ax2.grid(True, alpha=0.3)

    fig.suptitle(f'Phase 3: Deposit Rate Sweep (M={mass}, H={H})',
                 fontweight='bold')
    fig.tight_layout()
    fig.savefig(RESULTS_DIR / f'phase3_sweep.png', dpi=150)
    plt.close(fig)

    print(f"\n  Saved: {RESULTS_DIR / 'phase3_sweep.png'}")
    return results
...
def main():
    parser = argparse.ArgumentParser(
        description='v18: Self-Sustaining Gravity — Radiation vs Expansion')
    ...
    parser.add_argument('--sweep-deposit', action='store_true',
                        help='Phase 3: sweep deposit_rate')
    ...
    parser.add_argument('--radius', type=float, default=30.0,
                        help='Sphere radius for graph nodes (default 30.0)')
    ...
    parser.add_argument('--deposit-strength', type=float, default=0.00001,
                        help='Entity deposit rate (default 0.00001)')
    ...
    if args.phase0:
        experiment_phase0(n_nodes=args.n_nodes, k=args.k, H=args.H,
    ...
    if args.sweep_deposit:
        experiment_sweep_deposit(
            n_nodes=args.n_nodes, k=args.k, H=args.H,
            alpha_expand=args.alpha_expand,
            mass=args.binary_mass,
