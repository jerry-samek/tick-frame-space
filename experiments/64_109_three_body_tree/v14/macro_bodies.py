"""v14: Connector Bandwidth Gravity — Two-Endpoint Edge Rule.

Built on v13 (pathfinder). Key change: edges know their TWO endpoints only.

    de/dt = H / (1 + alpha * (gamma_A + gamma_B))

Each edge reads ONLY its own endpoints' gamma. No distant masses, no r.
The gamma field IS the gravitational potential. Edges near mass grow slowly
(high gamma at both endpoints). Edges far away grow fast (low gamma).
Frame rotation from the resulting edge asymmetry produces geodesic orbits.

No gradient_coupling. No hand-set profiles. One local rule.
...
class ContinuousGammaField:
    ...
    def expand_edges(self):
        ...
        Stores per-edge growth in self.last_growth for local cascade drift.
        """
        if self.H <= 0:
...
class MacroBody:
    ...
    v14: cascade-only. No gradient_coupling. Movement comes from:
    1. Cascade drift — local edge growth asymmetry displaces the body
    2. Hop accumulator — body's own movement in its internal_direction
    3. Geodesic frame rotation — asymmetric edges tilt "forward" toward mass
    """
    ...
    def __init__(self, bid, node, mass=1.0, commit_mass=10,
        ...
        self.cascade_buffer = np.array([0.0, 0.0, 0.0])
    ...
    def advance(self, field, tick=None):
        """Advance one tick using local cascade + geodesic frame rotation.

        TWO SEPARATE MOTIONS:

        1. cascade_buffer: expansion drift from local edge growth asymmetry.
           Every tick, the difference in growth between opposing edges
           accumulates. When any component reaches +/-1, the body is
           physically displaced. The expanding space moves it.

        2. hop_accumulator + internal_direction: the body's OWN movement.
           At commit time, the body hops in its internal_direction.
           After each hop, geodesic frame rotation tilts "forward" toward
           shorter edges (toward mass).

        Both are local. Neither reads a gradient or knows about distant masses.
        """
        field.deposit(self.node, self.bid, self.mass * self.deposit_strength)

        # --- MOTION 1: Local expansion drift (every tick) ---
        cascade_moved = False
        if field.H > 0:
            # Drift from asymmetric edge growth at body's position.
            # last_growth[node, d] was computed by expand_edges() this tick.
            for ax in range(3):
                d_plus = ax * 2       # +x, +y, +z
                d_minus = ax * 2 + 1  # -x, -y, -z
                g_plus = field.last_growth[self.node, d_plus]
                g_minus = field.last_growth[self.node, d_minus]
                # More growth on one side → body displaced toward the other
                self.cascade_buffer[ax] += g_minus - g_plus
        ...
        # After hop: recompute "forward" relative to local edge lengths.
        # Asymmetric edges tilt the frame toward shorter edges (toward mass).
        # The body thinks it's going straight. The graph curves its path.
        tilt = np.zeros(3)
        for ax in range(3):
            d_plus = ax * 2       # +x, +y, +z
            d_minus = ax * 2 + 1  # -x, -y, -z
...
def run_verification():
    ...
    # Test 8: Cascade body hops at commit_mass interval
    print("Test 8: Hop timing with cascade advance")
    f8 = ContinuousGammaField(8000, k=6, G=0.0, body_ids=['A'])
...
def experiment_phase1(side=40, G=10.0, H=0.001, alpha_expand=1.0,
    ...
    print("=" * 70)
    print("PHASE 1: Two-Body with Emergent Metric (Two-Endpoint Rule)")
    print("=" * 70)
    ...
    # Tilt diagnostic at planet
    tilt_at_start = np.zeros(3)
    for ax in range(3):
        d_plus = ax * 2
        d_minus = ax * 2 + 1
        e_plus = field.edge_lengths[bodies[1].node, d_plus]
        e_minus = field.edge_lengths[bodies[1].node, d_minus]
        e_sum = e_plus + e_minus
        if e_sum > 0:
            tilt_at_start[ax] = -(e_plus - e_minus) / e_sum
    print(f"  Tilt at planet: |tilt|={np.linalg.norm(tilt_at_start):.6f}")
    ...
    if records:
        plot_distances(records, ['AB'],
                       f'Two-Body (v14): star={star_mass}, planet={planet_mass}',
...
def experiment_phase2(side=60, G=1.0, H=0.001, alpha_expand=1.0,
                      mass=100.0, commit=10, deposit_strength=1.0,
    ...
    print("=" * 70)
    print("PHASE 2: Equal Mass Binary (Acid Test)")
    ...
    bodies = [
        MacroBody('A', node_a, mass=mass, commit_mass=commit,
                  deposit_strength=deposit_strength),
        MacroBody('B', node_b, mass=mass, commit_mass=commit,
                  deposit_strength=deposit_strength),
    ]
    # Opposite tangential directions — should orbit center of mass
    bodies[0].internal_direction = np.array([0.0, 1.0, 0.0])
    bodies[1].internal_direction = np.array([0.0, -1.0, 0.0])

    init_dist = field.hop_distance(bodies[0].node, bodies[1].node)
    print(f"\n  Lattice: side={side}, N={field.n_nodes}")
    print(f"  Mass A = Mass B = {mass}, commit={commit}")
    print(f"  Separation: {init_dist} hops")
    print(f"  H={H}, alpha_expand={alpha_expand}, G={G}")
...
def experiment_phase3(side=80, G=10.0, H=0.001, alpha_expand=1.0,
                      ...
                      star_commit=999999, planet_commit=5, moon_commit=1,
    ...
    bodies[1].internal_direction = np.array([0.0, 1.0, 0.0])
    bodies[2].internal_direction = np.array([0.0, 1.0, 0.0])
...
def main():
    parser = argparse.ArgumentParser(
        description='v14: Connector Bandwidth Gravity (Two-Endpoint Edge Rule)')

    parser.add_argument('--verify', action='store_true')
    parser.add_argument('--phase0', action='store_true',
                        help='Edge expansion verification')
    parser.add_argument('--phase1', action='store_true',
                        help='Two-body with emergent metric')
    ...
    parser.add_argument('--star-commit', type=int, default=999999)
    ...
    parser.add_argument('--binary-commit', type=int, default=10,
                        help='Commit mass for equal-mass binary')
