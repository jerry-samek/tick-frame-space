...
v15 proved: (1) momentum conservation works, (2) growth asymmetry produces gravity,
(3) the cubic lattice kills orbits. The lattice quantizes motion to 6 axes —
tangential components die, tilt is purely radial, angular momentum flips at
lattice corners.

v16 replaces the lattice with a random geometric graph. Same one-stupid-rule:

    For each connector: nudge += (mean_growth - this_growth) / inertia
    When any connector's nudge >= initial_mean_edge -> hop through it

On the lattice, connector 0 means +x everywhere — that's the bug. On the real
graph, connector 0 means something different at every node. The entity just
follows the laziest connector. Direction change is automatic because connector
IDs don't align between nodes.
...
class Entity:
    """Entity on a random geometric graph.

    v16: velocity/displacement split. Velocity is a persistent 3D vector
    ...
    Per-connector displacement is correct for graph motion: progress toward
    each neighbor is independent. After hopping, residual displacement is
    converted to a 3D vector, projected onto the best connector at the new
    node, and continues accumulating. The entity lives in the comoving frame.
    """
    ...
    def advance(self, graph, tick=None):
        ...
        """
        graph.deposit(self.node, self.bid, self.mass * self.deposit_rate)
...
def experiment_phase1(n_nodes=10000, k=6, G=0.0, H=0.1, alpha_expand=1.0,
                      ...
                      deposit_strength=1.0, separation=10.0,
    ...
    print(f"  deposit_strength={deposit_strength}, inertia={inertia}")
    ...
    if formation_ticks > 0:
        ...
        for ft in range(formation_ticks):
            graph.deposit(node_star, 'star', deposit_strength)
            graph.spread()
    ...
    t0 = time.time()
    for tick in range(ticks):
        # Continuous deposit (maintains steady-state gamma field)
        graph.deposit(bodies[0].node, bodies[0].bid, deposit_strength)
        graph.spread()
        graph.expand_edges()
        for body in bodies:
    ...
    # Plot: physical AND comoving distances
    if records:
    ...
    # Trajectory plots: physical and comoving
    if any(b.pos_history for b in bodies):
        ...
        for body in bodies:
            if body.pos_history:
                xs = [c[1] for c in body.pos_history]
                ys = [c[2] for c in body.pos_history]
                ax1.plot(xs, ys, '-', linewidth=0.8, alpha=0.7, label=body.bid)
                ax1.plot(xs[0], ys[0], 'o', markersize=10)
                ax1.plot(xs[-1], ys[-1], 's', markersize=8)
        ax1.set_aspect('equal')
        ax1.set_title('Physical Trajectories (XY)')
        ax1.set_xlabel('X')
        ax1.set_ylabel('Y')
        ax1.legend()
        ax1.grid(True, alpha=0.3)

        # Comoving trajectory (divide positions by scale factor at each time)
        for body in bodies:
            if body.pos_history:
                # Estimate scale factor for each recorded position
                xs_c, ys_c = [], []
                for i, (t, x, y, z) in enumerate(body.pos_history):
                    # Find scale factor for this tick from records
                    a_t = 1.0
                    for r in records:
                        if r['tick'] >= t:
                            a_t = r['a_scale']
                    ...
                    xs_c.append(x / a_t)
                    ys_c.append(y / a_t)
                ax2.plot(xs_c, ys_c, '-', linewidth=0.8, alpha=0.7,
                         label=body.bid)
                ax2.plot(xs_c[0], ys_c[0], 'o', markersize=10)
                ax2.plot(xs_c[-1], ys_c[-1], 's', markersize=8)
        ax2.set_aspect('equal')
        ax2.set_title('Comoving Trajectories (XY / a(t))')
        ax2.set_xlabel('X (comoving)')
        ax2.set_ylabel('Y (comoving)')
        ax2.legend()
        ax2.grid(True, alpha=0.3)
...
def experiment_phase2(n_nodes=10000, k=12, G=0.0, H=0.1, alpha_expand=1.0,
    ...
    if formation_ticks > 0:
        ...
        ft0 = time.time()
        for ft in range(formation_ticks):
            graph.deposit(node_a, 'A', deposit_strength)
            graph.deposit(node_b, 'B', deposit_strength)
            graph.spread()
        print(f"  Formation done in {time.time()-ft0:.1f}s")
    ...
    t0 = time.time()
    for tick in range(ticks):
        # Continuous deposit (both bodies)
        for body in bodies:
            graph.deposit(body.node, body.bid, deposit_strength)
        graph.spread()
        graph.expand_edges()
        for body in bodies:
            body.advance(graph, tick)
...
def main():
    parser = argparse.ArgumentParser(
        description='v16: Drop the Lattice — Real Graph, One Stupid Rule')
