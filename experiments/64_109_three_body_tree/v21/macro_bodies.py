"""v21: Force-on-Hop — Read connectors once per hop, coast between.

v18-v20 apply gravitational force every tick. An entity sits on a node for
~100 ticks between hops, reading the same connectors and accumulating force
each tick. This causes velocity runaway (v20 sep=10 drag=0: |v| 0.1->2.8).

v21 fixes this: read force ONCE when arriving at a node, then coast on pure
inertia until the next hop. This matches discrete graph physics (entity only
learns new information at new nodes) and acts like a symplectic integrator
(force at discrete steps, coast between).

Usage:
    python macro_bodies.py --verify
    python macro_bodies.py --phase1 --n-nodes 30000 --ticks 50000 --log-hops
