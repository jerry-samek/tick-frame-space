...
def run_dynamics(field_graph, entities, dynamics_ticks, log_interval=100,
                 ...
                 temporal=False, exclusion=False):
    """Main dynamics loop.
    ...
    If exclusion=True (temporal only): entities cannot hop to a node occupied by
    another entity. This prevents the d=0 singularity where mutual gradient drops
    to zero and entities freeze permanently.
...
def run_three_body(n_nodes=50000, k=6, beta=0.3, spread_fraction=None,
    ...
    print(f"  mass={mass} (path commitment = {mass} edges), exclusion={exclusion}")
...
def main():
    ...
    parser.add_argument(
        ...
        help="Path commitment length (number of edges per path). "
             "Mass = inertia: heavier entities commit to longer straight-line "
             "paths, preserving direction. mass=1 = turn every hop. "
             "mass=5 = committed for 5 hops before re-reading gradient.",
    )
    parser.add_argument(
        "--exclusion", action="store_true",
        help="Prevent entities from occupying the same node (temporal mode only). "
