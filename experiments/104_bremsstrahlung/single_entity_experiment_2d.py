...
K = 0.2       # total radiative power (tune this)
...
def local_slope(potential_fn, x, y):
    eps_s = 1e-3
    dVdx = (potential_fn(x+eps_s, y) - potential_fn(x-eps_s, y)) / (2*eps_s)
    dVdy = (potential_fn(x, y+eps_s) - potential_fn(x, y-eps_s)) / (2*eps_s)
...
def simulate(potential_fn, curvature_sign):
    ...
    for _ in range(steps):
        ...
        for p_x, p_y, p_dir_y, _ in photons:
            ...
            # PHYSICS-LIKE COUPLING: constant radiative power
            coupling_strength = K / (actual_photon_speed + eps)
            coupling_strength = np.clip(coupling_strength, 0.0, 1.0)
...
axs[0].set_title("Hill geometry – constant radiative power")
...
axs[1].set_title("Well geometry – constant radiative power")
