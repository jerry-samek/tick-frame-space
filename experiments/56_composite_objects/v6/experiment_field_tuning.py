"""
Field Confinement Tuning Experiment for V6 Phase 4

Tests different field confinement parameters to solve energy accumulation issue.
Goal: Reduce field coverage from 99.1% to <20% while maintaining pattern stability.

Parameters tested:
- Gamma-dependent creation threshold (creation_sensitivity)
- Field decay in low-gamma regions (decay_threshold, decay_rate)
- Hybrid approaches (both mechanisms)

Author: V6 Phase 4 Implementation
Date: 2026-01-25
"""
...
import math
import json
from config_v6 import get_field_confinement_configs
...
def run_field_tuning_experiment(config_name: str, config):
    """Run single field tuning experiment with given configuration."""
...
def main():
    """Run all field confinement tuning experiments."""

    print("V6 Phase 4: Field Confinement Parameter Tuning")
    print("=" * 70)
    print()
    print("Goal: Reduce field coverage from 99.1% to <20% while maintaining drift <10%")
    print()

    configs = get_field_confinement_configs()
    results = []

    for config_name, config in configs.items():
        result = run_field_tuning_experiment(config_name, config)
    ...
    print(f"{'Config':<20} {'Creation':>10} {'Decay Thr':>10} {'Decay Rate':>10} {'Drift %':>10} {'Final Cov %':>12}")
    print("-" * 70)
    ...
    # Find best configuration (lowest coverage with drift < 10%)
    valid_results = [r for r in results if r["drift_percent"] < 10.0]

    if valid_results:
        best = min(valid_results, key=lambda r: r["final_coverage"])
        print(f"Best configuration (lowest coverage with drift < 10%): {best['config_name']}")
        ...
        print("WARNING: No configuration achieved drift < 10%")
        print("Best by coverage:")
    ...
    # Save results
    with open("results/field_tuning_results.json", "w") as f:
        json.dump(results, f, indent=2)

    print("Results saved to results/field_tuning_results.json")
    print("=" * 70)
