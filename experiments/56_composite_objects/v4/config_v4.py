...
class ConfigV4:
    ...
    """
    Distance threshold for fragment-fragment collisions (spatial proximity).

    Fragments collide when their centers are within this distance.
    This parameter controls collision frequency and thermalization rate.
    """
...
def get_energy_diagnostic_config(jitter_scale: float = 0.5) -> ConfigV4:
    """
    Get configuration for energy balance diagnostic.

    Tests if reducing jitter strength stabilizes the system.
    Default jitter_scale=0.5 means jitter_strength = 0.0005 (half of original 0.001).

    Args:
        jitter_scale: Multiplier for jitter_strength (0.1 = 0.0001, 0.5 = 0.0005, 1.0 = 0.001)
    """
    config = ConfigV4()
    config.n_fragments = 50  # V3 baseline
    config.num_ticks = 100000  # Diagnostic test (shorter than 200k)
    config.snapshot_interval = 200
    config.detailed_snapshot_interval = 2000
    config.progress_report_interval = 1000  # Frequent energy monitoring
    config.jitter_strength = 0.001 * jitter_scale  # REDUCED from 0.001
    return config


def get_quantization_200k_config() -> ConfigV4:
    """
    Get configuration for 200k tick quantization study with validated energy balance.

    Uses V4 energy diagnostic validated parameters:
    - 50 fragments (V3 proven baseline)
    - jitter = 0.0005 (validated at 100k ticks)
    - 200k ticks (sufficient for quantization signatures to emerge)

    Expected outcomes:
    - Radial shell structure (quantized orbital levels)
    - Energy level gaps (forbidden energies)
    - Angular momentum convergence
    - Maxwell-Boltzmann velocity distribution
    """
    config = ConfigV4()
    config.n_fragments = 50
    config.num_ticks = 200000  # 2× diagnostic test length
    config.snapshot_interval = 200
    config.detailed_snapshot_interval = 2000
    config.progress_report_interval = 1000  # Monitor energy balance
    config.jitter_strength = 0.0005  # VALIDATED from energy diagnostic
    return config
