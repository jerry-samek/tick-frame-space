"""
V14 Multi-Layer Grid - Simplified grid without load/energy fields.

Removed from V13:
- load_field (reaction-diffusion dynamics)
- energy_field (observer capacity)

Time dilation now emerges from tick skipping based on gamma gradient,
not from complex reaction-diffusion fields.

Author: V14 Implementation
Date: 2026-01-31
Based on: V13 multi_layer_grid.py (simplified)
"""
...
class MultiLayerGrid:
    ...
    V14 simplification: No load_field or energy_field.
    Time dilation emerges from tick skipping based on gamma gradient.
    """
    ...
    def get_gamma_statistics(self) -> dict:
        ...
        Returns:
            Dict with gamma min, max, mean, sum, and gradient stats
        """
...
if __name__ == "__main__":
    ...
    print(f"  Gradient max: {gamma_stats['gradient_max']:.4f}")
    print(f"  Gradient mean: {gamma_stats['gradient_mean']:.6f}")
    print()
