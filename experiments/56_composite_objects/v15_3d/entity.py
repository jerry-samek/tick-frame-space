"""
V15-3D Entity - Entity with tick skipping for time dilation.

Adapted from V15 2D for 3D positions. The entity itself is dimension-agnostic
since position is just a tuple - this file is mostly unchanged from V15.

Each entity:
- Has a unique ID = birth tick
- Owns one layer in the MultiLayerGrid3D
- Is born at origin [0, 0, 0] (head prepending)
...
Date: 2026-02-01
Based on: V15 entity.py (position tuple works for any dimension)
"""
...
class Entity:
    ...
    layer_id: int
    """Layer index in MultiLayerGrid3D (same as entity_id)."""

    position: Tuple[int, int, int] = field(default_factory=lambda: (0, 0, 0))
    """Current position (center of pattern). Always [0,0,0] at birth in 3D."""
...
def create_entity(tick: int, layer_id: int, origin: Tuple[int, int, int] = (0, 0, 0)) -> Entity:
    ...
    Args:
        ...
        layer_id: Layer index in MultiLayerGrid3D
...
if __name__ == "__main__":
    print("V15-3D Entity Demo")
    print("=" * 70)

    # Create entities at different ticks with 3D positions
    e1 = create_entity(tick=1, layer_id=0, origin=(15, 15, 15))
    e2 = create_entity(tick=2, layer_id=1, origin=(15, 15, 15))
    e3 = create_entity(tick=3, layer_id=2, origin=(15, 15, 15))
    ...
    # Simulate e2 with no skipping (not in well)
    print("Simulating e2 outside well (no skipping):")
    for i in range(5):
        e2.act_tick()

    print(f"e2 after 5 ticks:")
    print(f"  Total acts: {e2.total_acts}")
    print(f"  Total skips: {e2.total_skips}")
    print(f"  Time dilation factor: {e2.time_dilation_factor:.2f}")
    print()
