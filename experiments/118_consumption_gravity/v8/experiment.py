...
Same n-gram stream filtering as v7, but LEARNING_WINDOW is no longer a
fixed constant.  Instead, each entity's learning window = max(1, birth_tick)
— bounded by the causal horizon at the time of its creation.

Physical basis: speed of light = 1 tick/tick.  An entity born at tick t
has at most t ticks of causal history to learn from.  At the start of
the universe (tick 0), the window is 1 — the entity can observe a single
token before it must crystallize.  Later entities get progressively
larger windows and learn more refined spectra.

Compared to v7 (fixed window=2000), this should:
- Make root nearly useless (window=1 → 1-token spectrum)
- Push filtering responsibility to children
- Show spectrum quality improving with depth
- Still distinguish structured from random data
"""
...
DATA = os.path.join(os.path.dirname(BASE), "v7", "data")  # reuse v7 data


# --- Data Generation (reused from v7) ---------------------------------------
...
class Entity:
    """Stream-filtering entity with causal-horizon learning window."""
    ...
    def __init__(self, name, depth=0, parent=None, birth_tick=0):
        ...
        self.learning_window = max(1, birth_tick)  # causal horizon
        ...
        # Spectrum coverage tracking (post-crystallization)
        self.post_hits = 0          # tokens matching spectrum after crystal
        self.post_total = 0         # total tokens seen after crystal
    ...
    def coverage(self):
        """Spectrum coverage: fraction of post-crystal traffic consumed."""
...
def process(entity, token, tick, all_entities, events):
    ...
    # Track post-crystallization coverage
    entity.post_total += 1
    ...
    # Forward ALL rejects to existing child (cascade)
    if entity.children:
...
def run_single(source_name, data, ngram_n, ticks):
    ...
    # Average spectrum coverage across crystallized entities
...
def plot_results(results):
    ...
    fig.suptitle(
        "Exp 118 v8: Causal Window \u2014 Learning Bounded by Light Cone\n"
        f"window=max(1,birth_tick)  COVERAGE={TARGET_COVERAGE}  "
    ...
    # --- Row 0, Col 0: Consumed Depth vs N ---
    ax = axes[0, 0]
    for src in sources:
    ...
    # --- Row 0, Col 1: Avg Coverage vs N ---
    ax = axes[0, 1]
    for src in sources:
        d = sorted([r for r in results if r['source'] == src],
                   key=lambda r: r['n'])
        ax.plot([r['n'] for r in d],
                [r['avg_coverage'] * 100 for r in d],
                f'-{markers[src]}', color=colors[src], label=src, lw=2)
    ax.set(xlabel='N-gram size', ylabel='Avg Coverage %',
           title='Spectrum Coverage vs N\n(how well entities learned)')
    ax.set_xticks(ns)
    ax.legend()
    ...
    plt.tight_layout()
    fig_path = os.path.join(OUT, 'v8_results.png')
...
def run():
    ...
    # Generate data sources if missing (reuses v7/data/ directory)
    ...
    # --- Summary CSV ---
    csv_path = os.path.join(OUT, 'v8_summary.csv')
    ...
    # --- Per-entity CSV ---
    ent_csv = os.path.join(OUT, 'v8_entities.csv')
