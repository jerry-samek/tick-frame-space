#!/usr/bin/env python3
"""
Experiment 118 v7 -- Phase 2c: Outer Orbital Test

Planet placed OUTSIDE star body: nodes where dist_from_star_COM > star_mean_r + 5.
Same tangential kick. Does the orbit survive with weaker gravitational signal?
"""
...
PLANET_MIN_DIST = None  # set after warmup: star_mean_r + 5
...
def run():
    ...
    # ── Place planet OUTSIDE star body ──
    star_r = star.mean_radius(g)
    min_dist = star_r + 5.0
    dists_from_star = np.linalg.norm(g.pos - star_com, axis=1)
    # Pick nodes closest to min_dist but all beyond it
    outside = np.where(dists_from_star >= min_dist)[0]
    sorted_outside = outside[np.argsort(dists_from_star[outside])]
    planet_node_ids = sorted_outside[:PLANET_COUNT].tolist()
    print(f"\n  Planet placement: star_r={star_r:.2f}, min_dist={min_dist:.2f}")
    ...
    csv_path = os.path.join(OUT, "phase2c_results.csv")
...
def plot(rows, initial_dist):
    ...
    fig.suptitle('Experiment 118 v7 -- Phase 2c: Orbital Test (tangential kick)',
                 fontsize=14)
    ...
    plt.tight_layout()
    png_path = os.path.join(OUT, "phase2c_results.png")
