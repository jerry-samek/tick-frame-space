...
# --- Barycentric clamp ---
def clamp_to_triangle(x, y, A, B, C):
...
# --- Rotate ALL points ---
def rotate_all_points(vertices, transform, tick):
    cx, cy = centroid_calc(vertices)
    A, B, C = vertices[0], vertices[1], vertices[2]

    rotated = []
    for x, y, t, c in vertices:
        sx, sy = x - cx, y - cy
        rx, ry = transform(sx, sy)
        X, Y = rx + cx, ry + cy

        # clamp to triangle
        X, Y = clamp_to_triangle(X, Y, A, B, C)

        rotated.append([X, Y, tick, c])
    return rotated


# --- Operators ---
def operator_cw(state, tick):
    state["vertices"] = rotate_all_points(state["vertices"], transform_cw, tick)
    return state


def operator_ccw(state, tick):
    state["vertices"] = rotate_all_points(state["vertices"], transform_ccw, tick)
    return state
...
def add_fill_point(state, tick):
    ...
    # clamp
    x, y = clamp_to_triangle(x, y, v1, v2, v3)
