...
def rotate_around_centroid(vertices, transform):
    cx, cy = centroid(vertices)
    shifted = [[x - cx, y - cy] for x, y in vertices]
    rotated = [transform(x, y) for x, y in shifted]
...
def operator_cw(state):
    state["vertices"] = rotate_around_centroid(state["vertices"], transform_cw)
    return state


def operator_ccw(state):
    state["vertices"] = rotate_around_centroid(state["vertices"], transform_ccw)
    return state
...
def operator_down(state):
    verts = state["vertices"]
    verts[0][1] += 1  # horní bod dolů o 1
    state["vertices"] = verts
    return state
...
def project(point, pos, scale, offset, zoom, depth, vanish_point):
    x, y = point
    x *= scale * zoom
    y *= scale * zoom
...
while True:
    ...
    for op in list(triangle_state["operators"]):
        triangle_state = operators[op](triangle_state)
    ...
    for i, past_state in enumerate(history):
        ...
        if render_mode >= 1:
            pygame.draw.polygon(screen, (150, 150, 150), projected, 1)
        if render_mode == 2:
            pygame.draw.polygon(screen, (50, 50, 200), projected, 0)
