"""
Experiment 128 v7 — Entities with Tangential Reject Stream

Star: same as v6 (gravity from accumulated density + forward default).
Planet: NEW routing — the reject stream goes TANGENTIAL.

When a planet node routes, it computes:
1. Gravity direction (toward star deposits — radial inward)
2. Tangential direction (perpendicular to gravity, consistent handedness)
3. Combined: forward + gravity + tangential_reject

The tangential component comes from the ABSORPTION ASYMMETRY:
the star-facing side absorbs more → the away/sideways side emits more.
The emission direction IS the reject stream direction.
"""
...
class EntityBatch:
    """Same as v6 but planet routing adds tangential reject bias."""
    ...
    def __init__(self, name, node_ids, n_groups, group_offset,
        ...
        self.tangential_strength = tangential_strength  # 0 for star, >0 for planet
    ...
    def tick_routing(self, rng):
        ...
        # GRAVITY: toward densest matching deposits
        gravity = (density[:, :, None] * nbr_dirs).sum(axis=1)  # (n, 3)

        # FORWARD: inertia from last hop
        fwd = self.last_length[routing_idx, None] * self.arrival_dir[routing_idx]
        has_fwd = self.has_arrival_dir[routing_idx]
        fwd[~has_fwd] = 0.0

        # TANGENTIAL REJECT: perpendicular to gravity direction
        if self.tangential_strength > 0:
            ...
            # Cross product with z-axis to get tangential (consistent handedness)
            # tangential = gravity_dir × z_hat (in XY plane, perpendicular to radial)
            z_hat = np.array([0, 0, 1], dtype=np.float32)
            tangential = np.cross(grav_dir, z_hat)  # (n, 3)
            ...
            # Scale tangential by gravity magnitude × strength
            tangential *= grav_norm * self.tangential_strength
        else:
            tangential = np.zeros_like(gravity)

        # COMBINE: forward + gravity + tangential reject
        combined = fwd + gravity + tangential
        cn = np.linalg.norm(combined, axis=1, keepdims=True)
        valid_c = (cn > 1e-15).flatten()
        combined[valid_c] /= cn[valid_c]
        no_combined = ~valid_c & has_fwd
        combined[no_combined] = self.arrival_dir[routing_idx[no_combined]]

        # Score neighbors
        alignment = (combined[:, None, :] * nbr_dirs).sum(axis=2)
