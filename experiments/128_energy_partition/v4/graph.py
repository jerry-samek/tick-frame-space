"""
Graph substrate for Experiment 118 v16.

The ONE change from v13: Same deposits CONSUME one Different deposit.
This makes connectors SHRINK from Same traffic and GROW from Different traffic.
The tug-of-war creates equilibrium distances.

  Same deposit → reinforce + consume one Different (different_count -= 1)
  Different deposit → extend (different_count += 1)

Connector length = initial + different_count. Can oscillate, not just grow.
"""
...
class Connector:
    """Connector with consumption: Same consumes Different."""

    __slots__ = ('initial_length', 'deposits', 'total', 'different_count',
                 'same_count', 'consumed_count',
    ...
    def __init__(self, initial_length):
        ...
        self.same_count = 0
        self.consumed_count = 0  # Different deposits consumed by Same
    ...
    @property
    def length(self):
        """Effective length = initial + net Different (can shrink!)."""
    ...
    def append(self, group_tag, src_node=None, dst_node=None):
        """Append deposit. Same consumes one Different. Different extends.

        The consumption IS transformation: Different -> Same reclassification.
        The deposit still exists but its contribution to length is neutralized.
        """
        ...
        if dominant is None:
            ...
            # Same family -> reinforces AND consumes one Different
            self.same_count += 1
...
class Graph:
    ...
    def total_same_different(self):
        ...
        total_consumed = sum(c.consumed_count for c in self.connectors.values())
        return total_same, total_diff, total_consumed
