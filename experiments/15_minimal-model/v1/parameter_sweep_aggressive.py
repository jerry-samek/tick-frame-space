"""
Aggressive parameter sweep with much higher emission strengths.
Goal: Find the threshold where agent commits actually occur.
"""
...
# -----------------------------
# Aggressive parameter sweep
# -----------------------------
alpha_0_values = [5.0, 10.0, 20.0, 50.0, 100.0]
alpha_1_values = [0.0, 5.0, 10.0]
        ...
        if Theta >= next_commit_threshold:
            ...
            if tick_count % M == 0:
                ...
                if Psi >= 1.0 + epsilon:
                    ...
                    if verbose:
                        print(f"    [AGENT COMMIT!] k={agent_frame_count} t={commit_time:.3f} Psi={Psi:.3f} S={S:.6f}")
                    Psi = 0.0
                else:
                    agent_log.append({
                        "FrameID": agent_frame_count,
...
# -----------------------------
print("=" * 80)
print("AGGRESSIVE PARAMETER SWEEP - SEARCHING FOR AGENT COMMITS")
print("=" * 80)
    ...
    for alpha_1 in alpha_1_values:
        for gamma in gamma_values:
            for M in M_values:
                ...
                # Once we find commits, we can reduce verbosity
                verbose = not first_commit_found

                result = run_simulation(alpha_0, alpha_1, gamma, M, verbose=verbose)

                if result is None:
                    print(f"[Run {run_number:03d}] SKIPPED: CFL violation")
                    continue

                results.append(result)
                stats = result['statistics']

                status = "***COMMITS***" if stats['agent_commit_count'] > 0 else ""
                print(f"[Run {run_number:03d}] a0={alpha_0:5.1f} a1={alpha_1:5.1f} g={gamma:.4f} M={M} | "
                      f"Ticks={stats['tick_count']:2d} AgentCommits={stats['agent_commit_count']:2d} "
                      f"MaxSal={stats['max_salience']:.6f} {status}")
...
if runs_with_commits:
    ...
    for result in runs_with_commits:
        p = result['parameters']
        s = result['statistics']
        print(f"  a0={p['alpha_0']:5.1f}, a1={p['alpha_1']:5.1f}, g={p['gamma']:.4f}, M={p['M']} "
              f"-> {s['agent_commit_count']} commits, max_salience={s['max_salience']:.6f}")

# Save results to JSON
output_file = "parameter_sweep_aggressive_results.json"
