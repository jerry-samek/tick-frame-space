...
## Correction (2026-07-04, from Test 3)

The Core data sentence above — "damage persists as a bounded on-ring phase-shift; it does **not** collapse — contra the naive 'periodic attractor kills perturbations'" — is **wrong as a statement about the pattern**, and was the root of a false premise propagated into `PREREG_phase1b.md` §2 ("single ±1 kick (survivable...)"). Test 3's kick battery and time-to-death trace (`RESULTS_test3.md`) show: 112/128 single on-ring ±1 kicks KILL the K8 patterns (median 3 ticks, max 7, 100% before the W=48 readout horizon); only 2K phase-aligned kicks (those mimicking the rule's own next action) are absorbed. What persists is the **divergence field** (remnant − template), not the pattern: `frac_nonzero` ≈ 0.9 was read as "phase-shift persistence" when it is largely the signature of partial death remnants (6- and 4-cell) and, in 23% of the ensemble, of an empty canvas differenced against the memorized template. The naive expectation was in fact closer to the truth: the periodic attractor is fragile; the readout reads how it dies. Spatial confinement (§C) is unaffected — remnants are ring cells, which is precisely why this error stayed hidden.
