...
## Correction (2026-07-10, from the K=12 census)

Verdict 1's sentence — "two survivors with isomorphic local graphs would read bit-identically regardless of how they sit in Z³" — is **false**. The K=12 census (`RESULTS_k12.md`) found four survivors on the *identical* cell set (the 2×2×3 box; identical coupling graph by construction) whose M readings span 5.16–6.00, and the D1 battery showed M is not even representative-invariant (reversal shifts one member by 17 bootstrap-sd; lattice rotation shifts three members by 7–25). Root cause traced: the Exp 134 rule is isometry-covariant **only on tie-free states**; a ±1 kick immediately creates an |γ| tie (verified: first kick, tie at |γ|=12, divergence at tick 1), and ties are resolved by dict/set iteration order — implementation-dependent, not isometry-covariant. So the pipeline factors through the adjacency graph only *up to unspecified tie-breaking*. **This gate's discrimination results survive:** the G-M1 FLAT/CUBE gap was re-verified under representative, reversed, and rotated variants (+0.48/+0.57/+0.54, z ≥ 20, same sign) — rep-dependence at K=8 (~0.07) is an order of magnitude below the certified gap. T-B's positive result (abstract graphs reproduce the K8 pair's ordering) also stands; its worded conclusion "M reads coupling-graph structure" must be read as *sufficiency for that contrast*, not as M being a function of the graph.
