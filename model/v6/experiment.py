"""V6 Experiment runner: Orbital Program with Pygame visualization + headless mode.
...
def run_headless(args):
    ...
    print(f"V6 Headless (Orbital Program): radius={WORLD_RADIUS}, seeds={args.seeds}, "
    ...
    for tick in range(1, args.ticks + 1):
        ...
        if tick % 100 == 0:
            ...
            learning, execution, orbiting = world.phase_counts()
            ...
            print(f"  tick={tick:>6d}  entities={e_count:>6d}  "
                  f"learn={learning:>4d} exec={execution:>4d} orbit={orbiting:>4d}  "
                  ...
                  f"closures={world.orbit_closures}  "
                  f"failures={world.orbit_failures}  "
                  f"stays={world.stay_decisions}  "
                  f"brake={world.braking_emissions}  "
    ...
    print(f"Final: {world.entity_count()} entities "
          f"(learning={learning}, execution={execution}, orbiting={orbiting}), "
          f"system energy={world.total_system_energy():.1f}, "
          f"max generation={world.max_generation}")
    print(f"Orbit closures={world.orbit_closures}, "
          f"orbit failures={world.orbit_failures}")
...
def run_visual(args):
    ...
    pygame.display.set_caption("V6 Orbital Program (Memory as Trajectory)")
    ...
    while running:
        ...
        for e in world.entities:
            ...
            if e.phase == 'learning':
                # Learning phase: color by generation (same as V5)
                fill = e.memory_fill_pct
                brightness = int(80 + 175 * fill)
                ...
                # Execution phase: distinct visual
                if e.completed_cycles >= 1:
                    # Orbiting entity: bright white core with blue ring
                    pygame.draw.circle(screen, (100, 150, 255), (isx, isy), 5)
                    pygame.draw.circle(screen, (255, 255, 255), (isx, isy), 3)
                else:
                    # First execution cycle (not yet proven orbit): magenta
                    pygame.draw.circle(screen, (200, 80, 200), (isx, isy), 4)
                    pygame.draw.circle(screen, (255, 255, 255), (isx, isy), 5, 1)

            # Draw heading arrow (learning) or program direction (execution)
            if e.phase == 'learning':
                hx, hy = e.heading()
        ...
        if show_gamma_grad:
            draw_gradient_arrows(world.field, (255, 200, 0))
        if show_photon_grad:
            draw_gradient_arrows(world.photon_field, (255, 80, 80), negate=True)
        ...
        txt = font.render(
            f"learning={learning}  execution={execution}  orbiting={orbiting}  "
            f"repl={world.replication_events}  "
            f"closures={world.orbit_closures}  failures={world.orbit_failures}",
            True, (180, 220, 180)
        )
        screen.blit(txt, (10, hud_y))
        hud_y += 18
        ...
        for i, e in enumerate(world.entities[:5]):
            q, r = e.pos
            hx, hy = e.heading()
            phase_str = e.phase[0].upper()
            cycles_str = f" cyc={e.completed_cycles}" if e.phase == 'execution' else ""
            txt = font.render(
                f"#{e.id} ({q},{r}) {phase_str} mem={e.memory_cursor}/{e.memory_size} "
                f"gen={e.generation}{cycles_str}",
                True, (160, 160, 160)
            )
            screen.blit(txt, (10, hud_y))
            hud_y += 16
...
def main():
    parser = argparse.ArgumentParser(description="V6 Orbital Program Experiment (Memory as Trajectory)")
