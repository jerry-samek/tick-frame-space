...
class CSVLogger:
    def __init__(self, output_dir: str):
        ...
        self.entity_file = open(os.path.join(output_dir, "entities_2d.csv"), "w", newline="")
        self.summary_file = open(os.path.join(output_dir, "summary_2d.csv"), "w", newline="")
        ...
        self.entity_writer.writerow([
            "tick", "entity_id", "x", "y", "baseline_h", "current_h", "energy",
    ...
    def log_tick(self, tick: int, world):
        ...
        for e in entities:
            current_h = world.hill.height[e.y, e.x]
            mem_str = "".join(str(int(m)) for m in e.memory)
            self.entity_writer.writerow([
                tick, e.id, e.x, e.y, f"{e.baseline_h:.4f}", f"{current_h:.4f}",
