...
V5 entity model -- tick energy, memory, heading, imprint, photonic sensing:
  - Each tick: gain 1 energy from tick, deposit 1 to gamma (imprint)
  - Movement is FREE (1 cell/tick = c, surfing the gradient)
  - Direction from four-way blend: gamma gradient + heading + occupancy + anti-photon
...
class Entity:
    ...
    def choose_direction(self, gradient: tuple[float, float, float],
        ...
        Four-way blend by magnitude:
        - Gamma gradient: points toward high energy (attraction -- seek hills)
        - Heading: points where entity has been going (inertia)
        - Occupancy gradient: points toward free space (reproductive pressure)
        - Photon anti-gradient: points AWAY from entity clusters (competition avoidance)

        The photon gradient points toward entity concentrations. We negate its
        direction (keep magnitude) to create repulsion -- entities move away from
        where other entities are clustered.
