"""World: tick orchestration with photonic emission model.
...
Update order each tick:
  ...
  4. Diffusion    -- average with neighbors (scipy convolve) for both gamma and photon fields
  5. Movement     -- each entity: gamma gradient + heading + occupancy + anti-photon -> move 1 cell
...
Photon field carries entity-presence information (not energy).
"""
...
class World:
    """Orchestrates the tick-frame simulation with photonic emission."""
    ...
    def step(self) -> None:
        ...
        # Step 2b: Photonic emission -- entity radiates presence to photon field
        for e in self.entities:
            if e.alive:
        ...
        # Step 4: Diffusion -- smooth both fields (propagation at c = 1 cell/tick)
        self.field.diffuse()
        self.photon_field.diffuse()

        # Step 5: Movement -- gradient + heading + occupancy + anti-photon -> move 1 cell
