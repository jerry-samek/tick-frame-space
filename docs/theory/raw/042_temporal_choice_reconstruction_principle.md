...
## Definition
Decisions are encoded on the main timeline as inevitabilities. Observers reconstruct these ticks backward, while the Visualizer flips the reconstruction forward to enforce continuity and the illusion of choice. This process requires no special logic: each layer simply takes the head of the buffer and renders it according to its role.
