...
> In both cases, demands on refresh stability increase, mirroring relativistic time dilation.
...
- **Planck tick** \(t_P\) remains constant:
  ...
  \]
- **Invariant speed of information transfer:** \(c\) is preserved as the maximum framerate.
...
- Frame time:
  \[
  R_n = N_n \cdot t_{\text{eff}}(x,v,\rho,\mathcal{K})
  \]
...
- Modulation:
  \[
  F(x,v) = F_0(x) \cdot \Xi(v)
  \]
- As \(v \to c\), \(\Xi(v) \to 0\), so \(t_{\text{eff}} \to \infty\).  
  → More ticks or stronger gating are required to maintain coherence.
...
- Modulation:
  \[
  F(x,\rho,\mathcal{K}) = F_0(x) \cdot \Omega(\rho,\mathcal{K})
  \]
- As density/curvature grows, \(\Omega \uparrow\), so \(t_{\text{eff}} \uparrow\).  
  → Refresh stability demands increase in strong gravitational fields.
...
- Perceptual dilation in this framework:
  ...
  \]
- Relativity’s Lorentz factor (velocity):
  \[
  \gamma_{\text{rel}}(v) = \frac{1}{\sqrt{1 - \frac{v^2}{c^2}}}
  \]
- Gravitational time dilation (curvature):
  \[
