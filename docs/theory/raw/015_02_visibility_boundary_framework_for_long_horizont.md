...
To preserve scientific validity, we cap horizons where recession or effective motion outpaces the observer’s framerate,
preventing perceptual aliasing.  
...
- **Guard rails:**
    ...
    - **Audit logs:** Record \(v_{\text{eff}}, v_{\text{frame}}, d_{\text{vis}}, f_{\text{obs}}, \Delta t_
      {\text{frame}}\).
...
- **Policy note:**
    - Rule: We do not report salience or dynamics beyond the visibility horizon; such frames are flagged and excluded
      from physics summaries.
...
This framework ensures long-horizon experiments remain scientifically interpretable by enforcing a clear, auditable
visibility boundary.  
Physics may remain stable beyond the boundary, but observation must not: when effective motion exceeds the observer’s
resolvable budget, we stop, flag, or resample — and we log every decision.
