# Energy Constant Principle (Scientific Formulation)
...
- **Momentum as expenditure**  
  Collisions and movements are costs subtracted from normalized energy.
- **Temporal normalization principle**  
  Entities with different raw energies at different ticks can be equivalent surfers if their normalized values match.
- **Condensation and dissolution**  
  Blobs redistribute energy but do not break the invariant.
- **Entropy narrative**  
  Extinction events (e.g., OOM) mark saturation, but normalized energy remains bounded near zero.
