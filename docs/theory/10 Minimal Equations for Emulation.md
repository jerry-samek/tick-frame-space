...
This document defines the smallest, self‑contained set of equations to emulate the theory with a continuous root and discrete visualization. It implements three core components: a **tick generator**, a **continuous base model**, and an **observer/debugger** that collapses visualization into committed versions.
...
- **Collapse emission (E2):**  
  \[
  \Theta(t^{-})<n\ \text{and}\ \Theta(t)\ge n \;\Rightarrow\; \text{emit root collapse at } t=t_{n},\quad n\leftarrow n+1
  \]
...
- **Percept accumulator and commit (E6):**  
  \[
  \dot{\Psi}(t)=r_{a}(t)\,S\big(x(t),a(t),o(t)\big),\quad \Psi(t^{-})<1\ \land\ \Psi(t)\ge 1\Rightarrow \text{agent collapse at }t;\ \Psi\leftarrow 0
  \]
...
- The **Observer/Debugger** samples a subset of root collapses, accumulates salience (\(\Psi(t)\)), and commits agentic collapses (percepts) when thresholds are crossed.  
