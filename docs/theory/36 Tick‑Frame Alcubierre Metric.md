# Tick‑Frame Alcubierre Metric

**Warp Geometry in Observer‑Constructed Space**

## 1. Context

In the substrate‑tick universe, the substrate provides:

- A single global tick counter
- No geometry
- No history
...
Changing how many ticks the observer processes produces:

- Subjective time dilation
- Effective long‑distance travel
- Apparent spatial jumps

### 3.2 Energy Distribution

In this model, "energy" represents influence on reconstruction. Observers can:

- Concentrate energy (focus, computation, memory)
- Reduce energy elsewhere (pruning, compression)

This reshapes the internal metric.

### 3.3 Internal Coordinate Systems

Observers define their own coordinate transforms. Changing these transforms changes:
...
This is the basis of warp geometry.

## 4. Warp Transform (Conceptual Form)

For a position `x`, warp center `c`, and warp strength `s`:
...
- Local motion remains sub‑light
- Effective global motion increases
- No substrate rules are violated
- Geometry is observer‑relative
- Warp bubble = region of modified metric
